...
export class TransferCorporateAccountService {
  private apiUrl = 'https://api.knoldg.comm/api/company/transfer/account/invitation';
  private isLoadingSubject = new BehaviorSubject<boolean>(false);
  private searchInsighterUrl = 'https://api.knoldg.comm/api/insighter/search/insighter';
  ...
  verifyTransferInvitation(email: string, code: string, lang: string): Observable<any> {
    ...
    this.setLoading(true);
    return this.http.post<any>('https://api.knoldg.comm/api/company/transfer/account/verification', formData, { headers }).pipe(
      map((res) => res),
      catchError((error) => this.handleError(error)), 
      finalize(() => this.setLoading(false))
    );
