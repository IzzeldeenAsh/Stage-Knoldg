...
export class SentMeetingsService {
  private apiUrl = 'https://api.knoldg.comm/api/account/meeting/client/list';
  ...
  getAvailableHours(insighterUuid: string): Observable<AvailableHoursResponse> {
    const headers = this.getHeaders();
    const url = `https://api.knoldg.comm/api/account/meeting/available/hours/${insighterUuid}`;
  ...
  rescheduleMeeting(meetingUuid: string, rescheduleData: RescheduleRequest): Observable<any> {
    ...
    const url = `https://api.knoldg.comm/api/account/meeting/reschedule/${meetingUuid}`;
