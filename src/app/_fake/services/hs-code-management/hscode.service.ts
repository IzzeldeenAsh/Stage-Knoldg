...
export class HSCodeService {
  private apiUrl = 'https://api.foresigha.co/api/common/setting/hs-code/list';
  private createApi = 'https://api.foresigha.co/api/admin/setting/hs-code';
  private updateDeleteApi = 'https://api.foresigha.co/api/admin/setting/hs-code';
  ...
  getHSCodeByISIC(isicCodeId: number,language:string): Observable<HSCode[]> {
    ...
    this.setLoading(true);
    return this.http
      .get<HSCodeResponse>(`https://api.foresigha.co/api/common/setting/hs-code/isic-code/${isicCodeId}`, { headers })
      .pipe(
        map((res) => res.data),
        catchError((error) => this.handleError(error)),
        finalize(() => this.setLoading(false))
