...
export class NotificationsInnerComponent extends BaseComponent implements OnInit {
  ...
  markAsRead(notificationId: string, callback?: () => void): void {
    ...
    this.http.put(`https://api.foresigha.co/api/account/notification/read/${notificationId}`, {}, { headers })
      .subscribe(
        () => {
  ...
  private handleNotificationNavigation(notification: Notification): void {
    ...
    if (notification.type === 'knowledge' && notification.category) {
      ...
      const knowledgeUrl = `https://foresigha.co/${lang}/knowledge/${notification.category}/${notification.param || ''}?tab=ask`;
  ...
  //   const knowledgeUrl = `https://foresigha.co/${lang}/knowledge/${notification.category}/${notification.param || ''}?`;
