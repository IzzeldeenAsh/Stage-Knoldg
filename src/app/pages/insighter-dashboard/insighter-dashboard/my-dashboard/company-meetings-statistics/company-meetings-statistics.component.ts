...
import {
  ...
  CompanyOrderMeetingStatisticsResponse
} from 'src/app/_fake/services/company-account/company-account.service';
import { TranslationService } from 'src/app/modules/i18n';

type CompanyMeetingStatistics = CompanyOrderMeetingStatisticsResponse['data'];

export type CompanyMeetingsChartOptions = {
...
@Component({
  selector: 'app-company-meetings-statistics',
  templateUrl: './company-meetings-statistics.component.html',
  styleUrls: ['./company-meetings-statistics.component.scss']
})
export class CompanyMeetingsStatisticsComponent implements OnInit, OnDestroy {
  @ViewChild('chart') chart: ChartComponent | undefined;

  private readonly chartColors = ['#0EA5E9', '#22D3EE', '#3B82F6', '#6366F1', '#10B981', '#F59E0B', '#F97316'];

  chartOptions: Partial<CompanyMeetingsChartOptions> = this.createInitialChartOptions();
  loading = false;
  error = false;
  hasData = false;
  ...
  private loadStatistics(): void {
    ...
    this.companyAccountService
      .getCompanyOrderMeetingStatistics()
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (data: CompanyMeetingStatistics) => {
          this.loading = false;
          this.error = false;

          this.totalMeetings = data?.total_orders ?? 0;
        ...
        error: (err) => {
          console.error('Error loading company meeting statistics', err);
          this.loading = false;
          this.error = true;
          this.insighters = [];
  ...
  private updateChartData(): void {
    ...
    this.chartOptions = {
      ...
      series: [
        {
          name: this.translationService.getTranslation('COMPANY_MEETINGS_STATISTICS.TOTAL_MEETINGS_LABEL'),
          data
        }
      ],
      xaxis: {
  ...
  private updateChartTranslations(): void {
    ...
    const label = this.translationService.getTranslation('COMPANY_MEETINGS_STATISTICS.TOTAL_MEETINGS_LABEL');
  ...
  private createInitialChartOptions(): Partial<CompanyMeetingsChartOptions> {
    return {
      series: [],
      chart: {
        type: 'bar',
