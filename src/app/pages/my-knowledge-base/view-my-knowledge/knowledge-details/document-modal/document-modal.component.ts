...
export class DocumentModalComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  editorInstance: Quill | null = null;
  ...
  generateAIAbstract(): void {
    ...
    const currentContent = this.editorInstance?.getText() || this.documentForm.get('description')?.value;
      ...
      next: (response: any) => {
        if (response.data) {
          ...
          if (summary && summary.trim() !== '') {
            ...
            // Then update the Quill editor with proper content setting
            setTimeout(() => {
  ...
  onEditorCreated(quill: Quill): void {
    ...
    // Set initial content if editing (only if not generated by AI)
    if (this.editingDocument && this.editingDocument.description && !this.hasGeneratedAbstract) {
      setTimeout(() => {
        if (this.editorInstance && this.editingDocument?.description && !this.hasGeneratedAbstract) {
          this.editorInstance.clipboard.dangerouslyPasteHTML(this.editingDocument.description);
        }
      }, 100);
    }
  }
  ...
  onContentChanged(): void {
    // Content change is automatically handled by reactive forms
    // This method can be used for additional logic if needed
  }
  
  private destroyEditor(): void {
    // Quill doesn't need explicit destruction like TinyMCE
    // The component lifecycle handles cleanup automatically
  ...
  private handleLanguageMismatchError(): Promise<boolean> {
    return new Promise((resolve) => {
      Swal.fire({
        title: this.lang === 'ar' ? 'عدم تطابق اللغة' : 'Language Mismatch',
        text: this.lang === 'ar' 
          ? `هناك عدم تطابق في اللغة بين اسم الملف ومحتواه. هل أنت متأكد من المتابعة؟`
          : `There is a language mismatch between the file name and its content. Are you sure you want to continue?`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: this.lang === 'ar' ? 'نعم، متابعة' : 'Yes, Continue',
        cancelButtonText: this.lang === 'ar' ? 'إلغاء' : 'Cancel',
  ...
  // Modal controls
  closeModal(): void {
    // Destroy TinyMCE editor before closing
    this.destroyEditor();
