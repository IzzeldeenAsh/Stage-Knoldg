...
import { UpgradeToCompanyComponent } from './upgrade-to-company/upgrade-to-company.component';
  ...
  getInvalidControls(formGroup: FormGroup | FormArray, parentKey: string = ''): string[] {
    ...
    Object.keys(formGroup.controls).forEach(key => {
      ...
      if (control instanceof FormGroup || control instanceof FormArray) {
        invalidControls = invalidControls.concat(this.getInvalidControls(control, controlName));
  ...
  get isSubmitDisabled():boolean{
    ...
    onSubmit() {
      ...
      if(this.hasRole(['client']) && !this.hasRole(['company']) && !this.hasRole(['insighter'])){
        ...
        console.log("this.accountForm.get('country_id')?.value.id",this.accountForm.get('country_id')?.value);
    ...
    postProfileAPI(formData:FormData){
      const postprofileAPISub = this._profilePost.postProfile(formData)
      .subscribe(
        {
          next:(res)=>{
