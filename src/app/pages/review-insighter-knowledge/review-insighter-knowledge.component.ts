...
export class ReviewInsighterKnowledgeComponent extends BaseComponent implements OnInit {
  ...
  public loadKnowledgeData(): void {
    ...
    const knowledgeSubscription = this.http.get<any>(`https://api.foresigha.co/api/company/library/knowledge/${this.knowledgeId}`, { headers })
      .subscribe({
        next: (response) => {
  ...
  submitDecision(status: 'approve' | 'decline'): void {
    ...
    const apiEndpoint = `https://api.foresigha.co/api/company/insighter/request/knowledge/accept/${targetRequestId}`;
